...
cd Astrafy-take-home-challenge/dbt_astrafy
...
├── README.md
├── excel_to_csv.py                  # Convertit les xlsx en CSV pour les seeds dbt
│
├── origin_data/
...
│   │   └── sales.csv
│   ├── analyses/
│   │   ├── q1_nb_orders_2023.sql
│   │   ├── q2_orders_per_month_2023.sql
│   │   ├── q3_avg_products_per_order_2023.sql
...
│
├── lookml/
│   ├── manifest.lkml               # Constante gcp_project
│   ├── models/
│   │   └── ecommerce.model.lkml
│   └── views/
│       ├── orders_segmented.view.lkml
│       ├── sales.view.lkml
│       └── sales_details.view.lkml
│
└── design/
...
```
---
***Le fichier excel_to_csv.py convertit les fichiers Excel du dossier origin_data/ en CSV dans le dossier seeds/ de dbt***
---
...
Les vérifications de qualité sont documentées dans `analyses/data_quality_checks.md`.
...
Choix techniques :
- Tous les identifiants (`order_id`, `customer_id`, `product_id`) sont castés en `STRING` dès le staging (**"cast early, cast once"**). Raisons : éviter les opérations arithmétiques accidentelles, anticiper les évolutions de format (ex. `ORD-12345`), et supprimer les cast implicites coûteux lors des JOINs dans BigQuery.
...
**`int_orders.sql`** — Exercice 4 : commandes 2022–2023 enrichies avec la quantité de produits par commande (`qty_product`).

**`int_orders_segmented.sql`** — Exercice 5 : logique de segmentation client sur fenêtre glissante 12 mois.

**`int_sales.sql`** — Renommage et sélection des colonnes depuis `stg_sales`, sert de base à `fct_sales`.
...
| Modèle | Description |
|--------|-------------|
| `fct_sales.sql` | Lignes de vente 2023 (1 ligne par order x product) |
...
Le projet GCP est centralisé via une constante dans `manifest.lkml` (`@{gcp_project}`), évitant le hardcoding du project ID dans chaque view.

### Structure

**`orders_segmented.view.lkml`** — basée sur `fct_orders_segmented`
- Dimensions : `order_id` (PK), `customer_id`, `order_date` (dimension_group avec timeframes), `order_segmentation`, `net_sales`
- Measures : `count_orders`, `count_customers` (count_distinct), `total_net_sales`

**`sales.view.lkml`** — basée sur `fct_sales`
- Dimensions : `order_id`, `product_id`, `customer_id`, `order_date`, `net_sales`
- Measures : `total_net_sales`, `total_qty`, `count_distinct_orders`, `count_distinct_product`, `avg_qty_per_order`

**`sales_details.view.lkml`** — derived table (explore_source) agrégée par client
- Dimensions : `customer_id` (PK), `count_distinct_orders`, `count_distinct_products`, `total_qty`, `total_net_sales`, `average_order_revenue`, `average_qty_per_order`

**`ecommerce.model.lkml`**
- Un seul explore `orders_segmented` avec joins `left_outer` sur `sales` et `sales_details`
- Relationship `one_to_many` sur `order_id` (1 commande → plusieurs produits)
- Relationship `many_to_one` sur `customer_id` (plusieurs commandes → 1 profil client)
